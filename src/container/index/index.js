export class Todo {
 ...
 this.#loadData()
    ...
    static #createTaskElem = (data) => {
       ...
       const [id, text, btDo, btnCancel] = el.children
       id.innerText = `${data.id}.`
       text.innerText = data.text
       btnCancel.onclick = this.#handleCancel(data)
       btDo.onclick = this.#handleDo(data, btDo, el)
       if(data.done) {
        el.classList.add('task--done')
        btDo.classList.remove('task__button--do')
        btDo.classList.add('task__button--done')
       }
